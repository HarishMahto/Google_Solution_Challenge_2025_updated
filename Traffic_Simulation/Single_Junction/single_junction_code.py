...
densityCoods = [(480,180),(880,180),(880,520),(480,520)]  # New coordinates for density display
...
# Road length for density calculation
roadLengths = {'right': 580, 'down': 320, 'left': 590, 'up': 255}
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        # Use absolute paths for loading images
        path = os.path.join(IMAGE_DIR, direction, vehicleClass + ".png")
...
# Calculate the number of vehicles and density for a given direction
def calculateDensity(direction):
    vehicle_count = 0
    for lane in range(3):
    ...
    # Calculate density (vehicles per unit of road length)
    density = 0
    if roadLengths[direction] > 0:
        density = vehicle_count / roadLengths[direction]
    
    return vehicle_count, round(density, 2)
...
# Set time according to formula and vehicle density
def setTime():
    ...
    try:
        ...
        pass  # Skip if 'say' command is not available

    # Get vehicle count and density for next direction
    direction = directionNumbers[nextGreen]
    vehicle_count, density = calculateDensity(direction)
    ...
    # Calculate green time based on vehicle count, types, and density
    # Using density as a multiplier to give more weight to congested roads
    density_factor = max(1.0, density * 10)  # Adjust scale as needed
    greenTime = math.ceil(((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + 
                         (noOfBuses*busTime) + (noOfTrucks*truckTime) + 
                         (noOfBikes*bikeTime)) / (noOfLanes+1) * density_factor)
...
def main():
    ...
    background = pygame.image.load(os.path.join(IMAGE_DIR, 'mod_int.png'))

    screen = pygame.display.set_mode(screenSize)
    pygame.display.set_caption("SIMULATION")

    # Loading signal images and font
    redSignal = pygame.image.load(os.path.join(IMAGE_DIR, 'signals', 'red.png'))
    yellowSignal = pygame.image.load(os.path.join(IMAGE_DIR, 'signals', 'yellow.png'))
    greenSignal = pygame.image.load(os.path.join(IMAGE_DIR, 'signals', 'green.png'))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
            # Add event handling for ESC key press
        ...
        # Calculate and display vehicle counts and densities for each direction
        densityTexts = []
        for i in range(0, noOfSignals):
            direction = directionNumbers[i]
            vehicle_count, density = calculateDensity(direction)
            density_text = f"V:{vehicle_count} D:{density}"
            densityTexts.append(font.render(density_text, True, black, white))
            screen.blit(densityTexts[i], densityCoods[i])
